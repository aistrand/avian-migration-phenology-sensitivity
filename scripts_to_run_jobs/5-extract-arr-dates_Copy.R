...
IAR_out_dir <- 'arrival_IAR_hm_2022-05-13_5000'

# master_out_dir <- 'arrival_master_2020-07-21'

master_out_dir <- 'arrival_master_2022-05-13_5000'
...
setwd(paste0('/Volumes/TOSHIBA\ EXT/Data/'))

species <- read.table('IAR_species_list_Hirundo_rustica_Antrostomus_carolinensis.txt')[,1]
...
out <- data.frame(species = rep(NA, NROW(df_master)), cell = NA, 
                  cell_lat = NA, cell_lng = NA, year = NA, 
                  arr_GAM_mean = NA, arr_GAM_sd = NA, VALID_GAM = NA, 
                  arr_IAR_mean = NA, arr_IAR_sd = NA, 
                  sigma_beta0_mean = NA, sigma_beta0_sd = NA,
                  beta_gamma_mean = NA, beta_gamma_sd = NA, plmax = NA, 
...
for (i in 1:length(species))
{
  ...
  if (length(grep(paste0(sp, '-iar-hm-stan_output-', IAR_out_date, '.rds'), list.files())) > 0)
  {
    ...
    cellcenters <- dplyr::filter(cellcenters, grid_cell %in% t_cells)
...
ifelse(!dir.exists(paste0('/Volumes/TOSHIBA\ EXT/Data/Processed/', master_out_dir)),
       dir.create(paste0('/Volumes/TOSHIBA\ EXT/Data/Processed/', master_out_dir)),
       FALSE)

# setwd(paste0(dir, 'Bird_Phenology/Data/Processed/', master_out_dir))

setwd(paste0('/Volumes/TOSHIBA\ EXT/Data/Processed/', master_out_dir))
