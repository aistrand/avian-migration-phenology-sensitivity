...
args <- c("Antrostomus_carolinensis", 5000)

# args <- c("Hirundo_rustica", 10)

# args <- c("Antrostomus_carolinensis", 10)
...
IAR_in_dir <- "IAR_input_2022-02-05"

# # IAR_out_dir <- 'arrival_IAR_hm_2020-07-21'

# OSCER

# IAR_out_dir <- paste0("arrival_IAR_hm_", args[3], "_", args[2])

IAR_out_dir <- 'arrival_IAR_hm_2022-05-10'
...
# setwd(paste0("/scratch/aistrand/Bird_Phenology/Data/Processed/", IAR_in_dir))

setwd(paste0('/Volumes/TOSHIBA\ EXT/Data/Processed/', IAR_in_dir))
...
f_out_2010 <- head(f_out, 30)

print(nrow(f_out_2010))

f_out_2011 <- f_out[109:138,]

print(nrow(f_out_2011))

f_out <- rbind(f_out_2010, f_out_2011)
...
setwd("/Users/alvastrand/Library/Mobile Documents/com~apple~CloudDocs/Documents/OU/Research/data/eBird/ebd_output/")

filename <- "ebd_US_states_east_Mississippi_0101_0731_complete_zerofilled_grid_cell_centers_relApr-2020.csv"

cellcenters <- read.csv(filename)

# print(nrow(cellcenters))

cellcenters <- dplyr::filter(cellcenters, grid_cell %in% cells)
...
#see script here showing this value of sigma_y_in has no impact on y_true: Archive/test_sigma_insert.R
...
print(max(rhat_output))

print(min(neff_output))
...
# setwd(paste0('/scratch/aistrand/Bird_Phenology/Data/Processed/', IAR_out_dir))

setwd(paste0('/Volumes/TOSHIBA\ EXT/Data/Processed/', IAR_out_dir))
